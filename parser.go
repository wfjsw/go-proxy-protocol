...
func consumeProxyLineV2(reader *bufio.Reader) (*ProxyLine, error) {
	...
	case 0x22:
		ret.Protocol = UDP6
	case 0x31:
		ret.Protocol = UNIXSTREAM
	case 0x32:
		ret.Protocol = UNIXDGRAM
	default:
	...
	if len > 0 {
		...
		if ret.Protocol == TCP4 || ret.Protocol == UDP4 || ret.Protocol == TCP6 || ret.Protocol == UDP6 {
			...
			if ret.Protocol == TCP4 || ret.Protocol == UDP4 {
				src_ip = net.IPv4(payload[0], payload[1], payload[2], payload[3])
				dst_ip = net.IPv4(payload[4], payload[5], payload[6], payload[7])
